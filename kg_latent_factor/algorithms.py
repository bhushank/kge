...
class Evaluater(object):
    ...
    def evaluate(self,params,x,num_samples):
        raise NotImplementedError()
...
class RankEvaluater(Evaluater):
    ...
    def evaluate(self,params,ex,num_samples=10):
        scores = self.objective.predict(params,ex,num_samples)
        if scores is None:
            return np.nan
...
class Objective(object):
    ...
    def cost(self,params,ex):
        ...
        if self.model_name in self.triple_models:
            ...
            cost += self.fprop_triple_models(x_s,x_t,t_negs,W_r,W,w)
    ...
    def predict(self,params,ex,num_samples=10):
        x_s, x_t, W_r = self.unpack_triple(params, ex)

        if self.model_name in self.triple_models:
            t_negs = self.neg_sampler.get_samples(ex, False,num_samples)

            if len(t_negs) > 0:
                # For negative targets
                negs = self.unpack_entities(params, t_negs)
                X_t_batch = np.append(x_t, negs, axis=1)
                if self.model_name == 'bilinear':
                    x_s = np.transpose(x_s)
                    return self.score(x_s, X_t_batch, W_r)
                elif self.model_name=='transE':
                    x_s = np.reshape(x_s, (x_s.shape[0],))
                    return self.score(x_s, X_t_batch.T, W_r)
                elif self.model_name == 'group interaction' or self.model_name=='holographic embedding':
                    W = self.unpack_params(params,'W')
                    return self.score(x_s, X_t_batch, W, W_r)
                elif self.model_name == 'er-mlp':
                    W = self.unpack_params(params, 'W')
                    w = self.unpack_params(params,'w')
                    return self.score(x_s, X_t_batch, W_r, W, w)
        ...
        if key[0] == 'e':
            ...
            if key[1] == 'W':
                ...
                if self.model_name=="holographic embedding":
                    ...
                    return util.to_floatX(self.param_scale*np.random.randn(self.e_d,3*self.e_d))
                else:
                    return NotImplementedError()
            if key[1] == 'w':
                return util.to_floatX(self.param_scale * np.random.randn(1,self.e_d))
    ...
    def bprop_triple_models(self,grad,ex,x_s,x_t,negs,negs_names,W_r,W,w,is_target=True):
        ...
        #y = np.zeros((1, X_t_batch.shape[1]), dtype=theano.config.floatX)
        #y[0, 0] = 1.0
        if self.model_name == 'bilinear':
            if is_target:
                x_s = np.transpose(x_s)
                ...
                #y = np.transpose(y)
                X_t_batch = np.transpose(X_t_batch)
                gx_s, gx_t, gW_r = self.bprop(X_t_batch, x_s, W_r)
                self.collect_entity_grads(grad, ex.t, gx_t)
            ...
            if is_target:
                x_s = np.reshape(x_s, (x_s.shape[0],))
                gx_s, gx_t, gW_r = self.bprop(x_s, X_t_batch.T, W_r)
                gx_s = np.reshape(gx_s, (1, gx_s.shape[0]))
                self.collect_entity_grads(grad, ex.s, gx_s)
                negs_names.insert(0, ex.t)
                self.collect_entity_grads(grad, negs_names, gx_t)
            else:
            ...
            if is_target:
                if self.model_name=='holographic embedding':
                    ...
                    gx_s, gx_t, gW_r = self.bprop(x_s, X_t_batch, W, W_r)
                elif self.model_name=='group interaction':

                    gx_s, gx_t, gW, gW_r = self.bprop(x_s, X_t_batch, W, W_r)
                    self.collect_param_grads(grad, 'W', gW)
                elif self.model_name == 'er-mlp':
                    gx_s, gx_t, gW_r, gW, gw = self.bprop(x_s, X_t_batch,W_r, W, w)
                    self.collect_param_grads(grad, 'W', gW)
                    self.collect_param_grads(grad, 'w', gw)
                ...
                if self.model_name == 'holographic embedding':
                    ...
                    gx_s, gx_t, gW_r = self.bprop(X_t_batch, x_s, W, W_r)
                elif self.model_name=='group interaction':

                    gx_s, gx_t, gW, gW_r = self.bprop(X_t_batch, x_s, W, W_r)
                    self.collect_param_grads(grad, 'W', gW)
                elif self.model_name == 'er-mlp':
                    gx_s, gx_t, gW_r, gW, gw = self.bprop(X_t_batch, x_s, W_r, W, w)
                    self.collect_param_grads(grad, 'W', gW)
    ...
    def fprop_triple_models(self,x_s,x_t,negs, W_r,W,w):
        '''
        ...
        if self.model_name == 'bilinear':
            ...
            x_s = np.reshape(x_s, (x_s.shape[0],))
            return self.fprop(x_s, X_t_batch.T, W_r)

        elif self.model_name=='group interaction' or self.model_name == 'holographic embedding':
            return self.fprop(x_s, X_t_batch, W, W_r)

        elif self.model_name == 'er-mlp':
            return self.fprop(x_s, X_t_batch, W_r, W, w)
